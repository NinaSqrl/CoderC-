...

{
    public class ProductoHandler
    {
        ...
        public void CrearProducto(Producto producto)
        {
            ...
            try
            {
                using (SqlCommand cmd = new SqlCommand("INSERT INTO Producto(id, Descripciones, Costo, PrecioVenta, Stock, IdUsuario) VALUES(@id, @descripciones, @costo, @precioVenta, @stock, @idUsuario); SELECT @@Identity", conexion))
                {
                    conexion.Open();
        ...
        public Producto ActualizarProducto(int id, Producto productoAActualizar)
        {
            ...
            try
            {
                Producto productoOriginal = ObtenerProducto(productoAActualizar.Id);
                if (productoOriginal == null)
                {
                    return null;
                }
                List<string> camposAActualizar = new List<string>();
                if (productoOriginal.Descripciones != productoAActualizar.Descripciones && !string.IsNullOrEmpty(productoAActualizar.Descripciones))
                {
                    camposAActualizar.Add("descripciones = @descripciones");
                    productoOriginal.Descripciones = productoAActualizar.Descripciones;
                }
                if (productoOriginal.Costo != productoAActualizar.Costo && productoAActualizar.Costo > 0)
                {
                    camposAActualizar.Add("costo = @costo");
                    productoOriginal.Costo = productoAActualizar.Costo;
                }
                if (productoOriginal.PrecioVenta != productoAActualizar.PrecioVenta && productoAActualizar.PrecioVenta > 0)
                {
                    camposAActualizar.Add("precioVenta = @precioVenta");
                    productoOriginal.PrecioVenta = productoAActualizar.PrecioVenta;
                }
                if (productoOriginal.Stock != productoAActualizar.Stock && productoAActualizar.Stock > 0)
                {
                    camposAActualizar.Add("Stock = @stock");
                    productoOriginal.Stock = productoAActualizar.Stock;
                }
